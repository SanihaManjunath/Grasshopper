...
for i in range(0, len(files), 2):
    ...
    if(num not in no_border):
        ...
        total_area_considered += ward_area
        ...
        total_green_area += cv2.countNonZero(mask)
        total_area_considered += height*width
        # print(location, "(no border): ", cv2.countNonZero(mask) / (height*width) * 100, "%") #gives same value as green_percentage

print("Average green area in bangalore:", total_green_area / total_area_considered * 100)
